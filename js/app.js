(() => {
        ...
        if (spollersArray.length > 0) {
            ...
            function setSpollerAction(e) {
                ...
                if (el.closest("[data-spoller]")) {
                    ...
                    const storiesItems = document.querySelectorAll(".graduated-stories__item");
                    const closest = el.closest("[data-story]");
                    const storyItemId = closest ? closest.dataset.story : null;
                    const storyItem = document.querySelector(`[data-user="${storyItemId}"]`);
                    if (!spollersBlock.querySelectorAll("._slide").length) {
                        if (oneSpoller && !spollerTitle.classList.contains("_spoller-active")) {
                            hideSpollersBody(spollersBlock);
                            storiesItems.forEach((storiesItem => {
                                storiesItem.classList.remove("_active");
                            }));
                        }
                        spollerTitle.classList.toggle("_spoller-active");
                        _slideToggle(spollerTitle.nextElementSibling, spollerSpeed);
                        document.querySelector(".stories__graduated").classList.add("_story-active");
                        storyItem ? storyItem.classList.toggle("_active") : null;
                    }
                    e.preventDefault();
                }
            }
    ...
    function initSliders() {
        if (document.querySelector(".quotation__slider")) new core(".quotation__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            ...
            loop: true,
            navigation: {
                prevEl: ".quotation__slider .swiper-button-prev",
                nextEl: ".quotation__slider .swiper-button-next"
            },
            on: {}
        });
    }
    var sliderblockSlider = null;
    var sliderblockALevelSlider = null;
    var mediaQuerySize = 479.98;
    function sliderblockSliderInit() {
        if (!sliderblockSlider) sliderblockSlider = new core(".sliderblock-tab__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 1.4,
            ...
            centeredSlides: true,
            navigation: {
                prevEl: ".sliderblock-tab__slider .swiper-button-prev",
                nextEl: ".sliderblock-tab__slider .swiper-button-next"
            },
            on: {}
        });
        if (!sliderblockALevelSlider) sliderblockALevelSlider = new core(".sliderblock-tab__slider-alevel", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 1.4,
            ...
            centeredSlides: true,
            navigation: {
                prevEl: ".sliderblock-tab__slider-alevel .swiper-button-prev",
                nextEl: ".sliderblock-tab__slider-alevel .swiper-button-next"
            },
            on: {}
        });
    }
    function sliderblockSliderDestroy() {
        if (sliderblockSlider) {
            sliderblockSlider.destroy();
            sliderblockSlider = null;
        }
        if (sliderblockALevelSlider) {
            sliderblockALevelSlider.destroy();
            sliderblockALevelSlider = null;
        }
    }
    window.addEventListener("load", (function(e) {
        initSliders();
        var windowWidth = window.innerWidth;
        if (windowWidth <= mediaQuerySize) sliderblockSliderInit();
    }));
    window.addEventListener("resize", (function(e) {
        var windowWidth = window.innerWidth;
        if (windowWidth <= mediaQuerySize) sliderblockSliderInit(); else sliderblockSliderDestroy();
    }));
    class ScrollWatcher {
        constructor(props) {
            let defaultConfig = {
    ...
    function windowLoad() {
        ...
        let sections = document.querySelectorAll(".spoller-results-2021");
        if (sections.length) sections.forEach((section => {
            observer.observe(section);
        }));
    }
    const storiesItems = document.querySelectorAll(".graduated-stories__item");
    var script_mediaQuerySize = 991.98;
    document.addEventListener("click", (function(e) {
        const windowWidth = window.innerWidth;
        if (windowWidth <= script_mediaQuerySize) {
            const targetElement = e.target;
            const closest = targetElement.closest("[data-user]");
            const spollerItemId = closest ? closest.dataset.user : null;
            const spollerItem = document.querySelector(`[data-story="${spollerItemId}"]`);
            const spollersBlock = spollerItem ? spollerItem.closest("[data-spollers]") : null;
            const closestStory = targetElement.closest("[data-story]");
            const storyItemId = closestStory ? closestStory.dataset.story : null;
            document.querySelector(`[data-user="${storyItemId}"]`);
            if (targetElement.closest(".graduated-stories__item")) if (!closest.classList.contains("_active")) {
                storiesItems.forEach((storiesItem => {
                    storiesItem.classList.remove("_active");
                }));
                hideSpollersBody(spollersBlock);
                ...
                script_slideToggle(spollerItem.nextElementSibling);
            } else {
                closest.classList.remove("_active");
                document.querySelector(".stories__graduated").classList.remove("_story-active");
                spollerItem.classList.remove("_spoller-active");
                script_slideUp(spollerItem.nextElementSibling);
            }
            function hideSpollersBody(spollersBlock) {
                const spollerActiveTitle = spollersBlock.querySelector("[data-spoller]._spoller-active");
                const spollerSpeed = spollersBlock.dataset.spollersSpeed ? parseInt(spollersBlock.dataset.spollersSpeed) : 500;
    ...
    let script_slideUp = (target, duration = 500, showmore = 0) => {
        if (!target.classList.contains("_slide")) {
            target.classList.add("_slide");
            target.style.transitionProperty = "height, margin, padding";
            target.style.transitionDuration = duration + "ms";
    ...
    let script_slideDown = (target, duration = 500, showmore = 0) => {
        if (!target.classList.contains("_slide")) {
            target.classList.add("_slide");
            target.hidden = target.hidden ? false : null;
            showmore ? target.style.removeProperty("height") : null;
